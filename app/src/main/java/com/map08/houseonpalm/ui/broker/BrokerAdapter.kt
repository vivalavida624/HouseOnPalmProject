...
class BrokerAdapter(
    ...
    class BrokerViewHolder(itemView: View, private val onEdit: (Broker) -> Unit, private val onDelete: (String) -> Unit) : RecyclerView.ViewHolder(itemView) {
        fun bind(broker: Broker) {
            ...
            itemView.findViewById<Button>(R.id.edit_button).setOnClickListener { onEdit(broker) }
            itemView.findViewById<Button>(R.id.delete_button).setOnClickListener { onDelete(broker.id) }
        }
    }
